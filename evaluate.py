...
def vllm_infer(
    ...
    template: str = "llama3_template_tune",
    cutoff_len: int = 2048,
    ...
    # train_name = model_args.adapter_name_or_path[0].split("/")[
    #     -1] if model_args.adapter_name_or_path else "full"
    # print(f"Generating predictions using {model_args.model_name_or_path} with {train_name} on {dataset} dataset.")
    ...
    # train_name = model_args.adapter_name_or_path[0].rstrip("/").split("/")[-1]
    # get subdirectory name from adapter_name_or_path
    train_name = "_".join(model_args.adapter_name_or_path[0].rstrip("/").split("/")[3:])
    test_name = dataset
    save_path = f"output/{model_name}/{train_name}/{test_name}"
    os.makedirs(save_path, exist_ok=True)
    with open(f"{save_path}/out.json", "w", encoding="utf-8") as f:
        for text, pred, label in zip(prompts, preds, labels):
            f.write(json.dumps({"prompt": text, "predict": pred, "label": label},
                               ensure_ascii=False) + "\n")
